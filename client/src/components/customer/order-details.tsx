...
import LoadingOrderDetails from "./loading-order-details";
...
const OrderDetails = ({id}: {id: number}) => {
  ...
  return (
    <div className='text-sm grid md:grid-cols-[3fr_2fr] gap-4'>
      <div className="space-y-2 md:space-y-4">
        <Card>
          <CardContent className='grid md:grid-cols-3 gap-1 md:gap-4'>
        ...
        <Card>
          <CardContent className='space-y-2'>
            ...
            <div className="flex gap-2 items-center justify-center">
              ...
              {order.steps.map((o, index) => (o.key === "delivered" && o.done && (
                <Button key={index}>Mark Complete</Button>
              )))}
              {order.steps.map((o, index) => ((o.key === "completed" || o.key === "delivered") && o.done && order.can_review && (
                <ReviewForm key={index} id={order?.id}/>
              )))}
            </div>
          </CardContent>
        </Card>
